...
async (conn, mek, m, { from, sender, reply }) => {
    try {
        ...
        if (!receivedMails || receivedMails.length === 0) {
            return await conn.sendMessage(from, { text: "EMPTY ➪ No Mails Here" }, { quoted: mek });
        }
...
async (conn, mek, m, { from, args, reply }) => {
    try {
        ...
        if (!match) {
            return await reply("Please provide a URL to shorten");
        }

        await reply("Shortening the link...");

        const url = await scraper.shortenUrl(match);

        if (url) {
            const msg = `Here's your shortened link: ${url}`;
...
async (conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        ...
        await reply("Fetching package details...");
